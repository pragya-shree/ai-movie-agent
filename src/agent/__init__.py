...
an agent orchestration layer, so that a conversational LLM (Qwen) can be
...
adds a decision/orchestration layer *around* it:
    ...
    user message
        ...
        -> LLMClient.chat()            (stub today, Qwen later)
...
Public API:
    ...
    LLMClient           - abstract interface Qwen will implement (see llm_client.py)
    get_tool_schemas    - Qwen/OpenAI-style function-calling schemas (see tools.py)
"""
